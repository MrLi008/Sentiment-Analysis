...
def register(request):
    ...
    if str(3424) == user_table_id:
        table_user_ins = mc_supermanager
    # 用户表

    if str(3418) == user_table_id:
...
def index(request):
    ...
    if request.method == "GET":
        ...
        tab_mc_predictions = {
            "params": mc_predictions().toParams_zh(),
            "records": mc_predictions.objects.all()[::-1],
            "json": [m.toJson() for m in mc_predictions.objects.all()[::-1]],
        }

        tab_mc_models = {
            "params": mc_models().toParams_zh(),
            "records": mc_models.objects.all()[::-1],
            "json": [m.toJson() for m in mc_models.objects.all()[::-1]],
        }

        tab_mc_trainingdata = {
            "params": mc_trainingdata().toParams_zh(),
            "records": mc_trainingdata.objects.all()[::-1],
            "json": [m.toJson() for m in mc_trainingdata.objects.all()[::-1]],
        }

        tab_mc_reviews = {
            "params": mc_reviews().toParams_zh(),
            "records": mc_reviews.objects.all()[::-1],
            "json": [m.toJson() for m in mc_reviews.objects.all()[::-1]],
        }

        tab_mc_movies = {
            "params": mc_movies().toParams_zh(),
            "records": mc_movies.objects.all()[::-1],
            "json": [m.toJson() for m in mc_movies.objects.all()[::-1]],
        }
